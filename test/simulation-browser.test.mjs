...
test('refreshed audit shows dates, elapsed model time and clearly inferred human time', { timeout: 60000 }, async () => {
  const page = await browser.newPage({ viewport: { width: 1440, height: 1000 } });
  const origin = base.replace(/simulation\/$/, '');
  const errors = [];
  page.on('pageerror', error => errors.push(error.message));
  await page.goto(`${origin}ai-usage.html`, { waitUntil: 'networkidle' });
  const metric = key => page.locator(`[data-audit-metric="${key}"] dd`);
  assert.equal(await metric('start-date').textContent(), '18 September 2026');
  assert.equal(await metric('last-update').textContent(), '20 September 2026');
  assert.equal(await metric('days').textContent(), '3');
  assert.equal(await metric('requests').textContent(), '1,164');
  assert.equal(await metric('models').textContent(), '2');
  assert.equal(await metric('agents').textContent(), '24');
  assert.equal(await metric('model-elapsed').textContent(), '10.50 h');
  assert.equal(await metric('human-time').textContent(), '2.76 h');
  assert.equal(await metric('engaged-time').textContent(), '13.26 h');
  assert.equal(await metric('model-work').textContent(), '11.67 h');
  assert.equal(await metric('cost').textContent(), '$888.84');
  assert.equal(await metric('author-review').textContent(), '3.46 h');
  assert.equal(await page.locator('#review-exposures').textContent(), '49,444');
  assert.match(await page.locator('#author-review').textContent(), /Do not add this estimate/);
  assert.equal(await page.locator('#review-documents input').count(), 9);
  assert.equal(await page.locator('#review-documents input:checked').count(), 1);
  for (const checkbox of await page.locator('#review-documents input').all()) await checkbox.check();
  assert.equal(await metric('author-review').textContent(), '6.50 h');
  await page.locator('#review-reset').click();
  await page.locator('#review-pace').fill('100');
  assert.equal(await metric('author-review').textContent(), '8.24 h');
  await page.locator('#review-additional').fill('60');
  assert.equal(await metric('author-review').textContent(), '9.24 h');
  await page.locator('#review-basis').selectOption('current');
  assert.equal(await metric('author-review').textContent(), '4.69 h');
  await page.locator('#review-reset').click();
  await page.locator('#review-coverage').fill('50');
  await page.locator('#review-rereads-max').fill('3');
  await page.locator('#review-rereads-min').fill('2');
  assert.equal(await metric('author-review').textContent(), '5.01 to 5.79 h');
  await page.locator('#review-reset').click();
  await page.locator('#review-pace').fill('');
  assert.equal(await page.locator('#review-error').isVisible(), true);
  assert.equal(await metric('author-review').textContent(), 'Assumptions incomplete');
  await page.locator('#review-reset').click();
  await page.locator('[data-review-document="main-report"]').uncheck();
  assert.match(await page.locator('#review-error').textContent(), /Select at least one/);
  await page.locator('#review-reset').click();
  assert.equal(await page.locator('#review-error').isVisible(), false);
  assert.equal(await metric('author-review').textContent(), '3.46 h');
  assert.equal(await metric('human-time').textContent(), '2.76 h');
  assert.equal(await metric('model-elapsed').textContent(), '10.50 h');
  const reviewHistory = await (await page.request.get(`${origin}review-history.json`)).json();
  assert.equal(reviewHistory.totals.reviewWordExposures, 92859);
  const reviewCorpus = await (await page.request.get(`${origin}author-review.json`)).json();
  assert.equal(reviewCorpus.documents[0].reviewWordExposures, 49444);
  assert.match(await page.locator('#usage-time').textContent(), /not a timesheet or a productivity estimate/);
  assert.equal(await page.locator('#usage-time tbody tr').count(), 4);
  assert.equal(await page.locator('#usage-models tbody tr').count(), 2);
  const historical = await (await page.request.get(`${origin}ai-usage-2026-09-19.json`)).json();
  assert.equal(historical.totals.requests, 401);
  const archivedConfig = await (await page.request.get(`${origin}usage-audit-config-2026-09-19.json`)).json();
  assert.equal(archivedConfig.cutoffExclusive, historical.scope.cutoffExclusive);
  const method = await (await page.request.get(`${origin}usage-method.md`)).text();
  assert.match(method, /\(\.\/ai-usage-2026-09-19\.json\)/);
  assert.match(method, /\(\.\/usage-audit-config-2026-09-19\.json\)/);
  for (const width of [1440, 760, 390]) {
    await page.setViewportSize({ width, height: 950 });
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1), true, `audit overflow at ${width}`);
  }
  assert.deepEqual(errors, []);
  await page.close();
});

test('author review has a static baseline without JavaScript and surfaces failed corpus loading', async () => {
  const origin = base.replace(/simulation\/$/, '');
  const context = await browser.newContext({ javaScriptEnabled: false });
  const staticPage = await context.newPage();
  await staticPage.goto(`${origin}ai-usage.html`);
  assert.equal(await staticPage.locator('[data-audit-metric="author-review"] dd').textContent(), '3.46 h');
  assert.match(await staticPage.locator('#review-speed-sensitivity').textContent(), /2\.75 to 4\.71 h/);
  assert.equal(await staticPage.locator('#author-review details').count(), 9);
  await context.close();
  const page = await browser.newPage();
  await page.route('**/author-review.json', route => route.fulfill({ status: 503, body: 'Unavailable' }));
  await page.goto(`${origin}ai-usage.html`, { waitUntil: 'networkidle' });
  assert.match(await page.locator('#review-error').textContent(), /HTTP 503/);
